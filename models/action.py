...
PROMPT = """This is a financial assistant system. This system is able to take corresponding action when user request. English and Vietnamese are supported. There are 2 possible system's action: TRANSFER[<receiver>,<amount>], CREATE_CHAT_GROUP[<user_comma_separated>].
...
def get_action_params(
        ...
        action: Literal["TRANSFER", "CREATE_CHAT_GROUP"],
    ) -> Dict[str, str]:
    """
    ...
    """
    assert action in ["TRANSFER", "CREATE_CHAT_GROUP"], f"Invalid action: {action}"
...
if __name__ == "__main__":
    ...
    out = get_action_params(messages=[
        ...
        {"user": "Hung", "message": "I want to create a chat group with Minh, Lan, and Cường"},
