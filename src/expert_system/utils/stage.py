...
def get_current_stage(model_output: str) -> Literal['Stage 1', 'Stage 2', 'Stage 3', 'Stage 4', 'BREAK']:
    ...
    'Stage 3'
    """
    current_stage = re.search(r"Current stage: (Stage \d)|(BREAK)", model_output)
    if current_stage is None:
        return 'BREAK'
    
    current_stage = current_stage.group(1) or current_stage.group(2)
...
if __name__ == "__main__":
    print(get_current_stage(" User is asking about the Current stage: BREAK"))
    print(get_current_stage(" User is asking about the Current stage: Stage 2"))